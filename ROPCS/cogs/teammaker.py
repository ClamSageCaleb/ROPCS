...
class Teammaker:
    ...
    #Takes the player who typed !q and either places them in the queue or not
    @commands.command(pass_context=True, name="queue", aliases=["q"], description="Add yourself to the queue")
    async def q(self, ctx):
        player = ctx.message.author
        #Intial queue will @here to get other players' attention
        if self.queue.qsize() == 0:
            await self.bot.say("@here \n {} wants to queue up! Type '!q' to join!\n".format(player.display_name))
        #Checks to see if queue is full
        if self.queue.qsize() != team_size:
            #Checks to see if player is already in queue
            ...
            #Formatting of the bots message which gives confirmation of joining the queue
            await self.bot.say("{} added to queue. ({:d}/{:d})".format(player.display_name, self.queue.qsize(), team_size))
            if self.queue.qsize() != team_size:
                await self.bot.say("Players in Queue:\n{}".format(", ".join([player.display_name for player in self.queue.queue])))
            if self.queue.qsize() == team_size:
                await self.bot.say("Players:\n{}".format(", ".join([player.mention for player in self.queue.queue])))
                await self.bot.say("Queue is now full! Type {prefix}captains or {prefix}voting or {prefix}random to create a game.".format(
                    prefix=self.bot.command_prefix))
        else:
            await self.bot.say("{}, queue is full, please try again in a few moments.".format(player.display_name))

    #Allows the player to leave the queue
    @commands.command(pass_context=True, name="deqeue", aliases=["leave"], description="Remove yourself from the queue")
    async def dq(self, ctx):
        player = ctx.message.author
        #Checks to see if plauyer is in the queue, if so removes them from the queue
        if player in self.queue:
    ...
    #Forcefully removes a player from a queue
    ...
    #change to say if self.queue.qsize() % 2 != 0 and if self.queue.qsize() % 2 == 0
    #also add !r alias
    ...
    async def random(self):
        ...
        #change 3 to self.queue.qsize() / 2
    ...
    async def display_teams(self):
        await self.bot.say("🔶 TEAM 1 🔶: {}".format(", ".join([player.mention for player in self.game.orange])))
        await self.bot.say("🔷 TEAM 2 🔷: {}".format(", ".join([player.mention for player in self.game.blue])))
        #PUT MOVE MEMBER HERE
